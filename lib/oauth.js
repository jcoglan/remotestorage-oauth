...
var authorize = function(target, clientId, scopes, options, callback) {
  ...
  var proxy = Proxy.createPool(function(response) {
    var hash = url.parse(response.headers.location || '').hash || '';
    
    if (response.statusCode < 301 || response.statusCode > 303 || !/\b(error|access_token)=/.test(hash))
      return false;
