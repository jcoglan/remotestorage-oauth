...
var authorize = function(target, clientId, scopes, options, callback) {
  ...
  var receiver = http.createServer(function(request, response) {
    Proxy.buffer(request, function() {
      ...
      if (request.method === 'GET') {
        fs.readFile(path.join(__dirname, '/receive.html'), function(error, content) {
          var body = ejs.render(content.toString('utf8'), {oauth_host: uri.host});
  ...
  server.listen(0, function() {
    var client = 'http://0.0.0.0:' + server.address().port,
